...
class SOPState:
    ...
    all_done: bool = False
    blocked: bool = False
    blocked_on_step: Optional[int] = None
...
class SOPTracker:
    ...
    def update(self, gpt_step: str, gpt_status: str, reason: str = "") -> SOPState:
        ...
        STRICT RULES:
          1. If the system is blocked, ONLY the blocked step can unblock it.
          2. If GPT reports a step that is NOT the next expected one, fire alert.
    ...
    def get_checklist(self) -> List[dict]:
        """
        Return the full SOP as a checklist.
    ...
    def _handle_missing(self, step_idx: int, reason: str):
        expected_next = self.state.last_completed_index + 1
        block_target = min(step_idx, expected_next)
        self.state.blocked = True
