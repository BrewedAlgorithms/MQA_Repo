...
structured response with:
  ...
  Core fields (step logic — unchanged):
    STEP / STATUS / REASON / ACTION

  NEW safety fields (parallel check, never affects step logic):
    SAFETY_OK  : true | false
    SAFETY_MSG : one-line message to show the worker if safety not followed
                 (empty string when SAFETY_OK is true)

Safety rules:
  - SAFETY_OK: false just means worker is not following safety gear/rules
    for the CURRENT step. The step itself still completes normally.
  - SAFETY_MSG is shown as a live warning banner to the worker.
  - If the current step has NO safety requirements, SAFETY_OK is always true.
"""
...
# Response schema to force structured output from the model
RESPONSE_FORMAT = {
    "type": "json_schema",
    "json_schema": {
        "name": "qa_inspection",
...
def build_user_prompt(
    ...
    Args:
        current_step_safety: safety requirements for the NEXT expected step.
                             Pass None or [] if there are no requirements.
    """
    sop_formatted = "\n".join(
...
class GptVisionInspector:
    ...
    def inspect_frame(
        ...
        Args:
            current_step_safety: list of safety strings for the current step,
                                 e.g. ["wear helmet", "dont wear gloves"].
                                 Pass None or [] if no safety requirements.

        Returns:
            Structured string with all 6 fields:
            STEP / STATUS / REASON / ACTION / SAFETY_OK / SAFETY_MSG
        """
        ...
        try:
            response = self.client.chat.completions.create(
                ...
                max_tokens=150,   # increased slightly to fit 2 extra fields
                temperature=0,
            )
        ...
        # First try to parse JSON (preferred path enforced via response_format)
        try:
        ...
        # Explicitly catch refusal-style replies and turn them into a structured error
        ...
        # Fallback: try to salvage — maybe GPT returned 4 fields without safety
        core_required = ["STEP:", "STATUS:", "REASON:", "ACTION:"]
        if all(any(l.startswith(r) for l in lines) for r in core_required):
            # Append default safe values and return
            return "\n".join(lines) + "\nSAFETY_OK: true\nSAFETY_MSG: "
    ...
    def parse_response(self, response_text: str) -> dict:
        ...
        Keys:
            step        — step number string or "Unknown"
            status      — OK | MISSING | OUT_OF_ORDER | UNKNOWN | ERROR
            reason      — one sentence
            action      — one instruction for operator
            safety_ok   — True | False  (bool)
            safety_msg  — warning message for worker, or "" if safe
        """
        result = {
            "step": "Unknown",
            "status": "UNKNOWN",
            "reason": "",
            "action": "",
            "safety_ok": True,      # default safe when uncertain
            "safety_msg": "",
        }
    ...
    def _format_dict_response(payload: dict) -> Optional[str]:
        ...
        # Minimal validation to ensure all core fields exist
